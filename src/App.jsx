...
export default function App() {
  ...
  const handleSwapTap = useCallback((id) => {
    if (!swapFirst) {
      // first tap — highlight it
      selectImage(id)
      setSwapFirst(id)
      return
    }
    if (swapFirst === id) {
      // tapped same image — cancel selection
      clearSelection()
      setSwapFirst(null)
      return
    }
    // second tap — swap content between the two slots
    saveSnapshot()
    swapImages(swapFirst, id)
    setSwapFirst(null)
    setSwapMode(false)
    clearSelection()
  }, [swapFirst, selectImage, clearSelection, saveSnapshot, swapImages])
  ...
  const toggleSwapMode = useCallback(() => {
    setSwapMode(m => {
      if (m) { setSwapFirst(null); clearSelection() }
      return !m
    })
  }, [clearSelection])
