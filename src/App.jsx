...
export default function App() {
  ...
  const [autoCollageOpen,   setAutoCollageOpen]    = useState(false)
  ...
  const handleEnhance = async () => {
    if (!images.length) return
    try {
      const { exportCollage } = await import('./utils/imageUtils')
      const sortedImages = [...images].sort((a, b) => (a.zIndex ?? 0) - (b.zIndex ?? 0))
      const blob = await exportCollage(sortedImages, backgroundColor, profileImage || null)
      setUpscaleBlob(blob)
    } catch {}
    setUpscaleOpen(true)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#0e0e1a', color: '#c8c8e8', overflow: 'hidden' }}>
      ...
      {/* ── Arrange Panel ────────────────────────────────────────────────── */}
      {autoCollageOpen && (
        <AutoCollage onClose={() => setAutoCollageOpen(false)} />
      )}
      ...
      <header style={{ background: '#13131f', borderBottom: '1px solid #1e1e2e', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <button style={btnPrimary} onClick={() => setHeroPickerOpen(true)}>+ Create</button>
            ...
            <button style={{ ...btnPrimary, background: 'linear-gradient(135deg,#06b6d4,#3b82f6)' }} onClick={handleEnhance}>✨ Enhance</button>
