...
function CanvasImage({ img, isSelected, onSelect }) {
  ...
  // Rendered size of this slot on the canvas
  const renderedW = img.naturalWidth  * img.scaleX
  const renderedH = img.naturalHeight * img.scaleY
  // Cell is square — use the smaller dimension
  const cellSize  = Math.min(renderedW, renderedH)

  // Center-crop the source image to a square
  const srcMin = Math.min(img.naturalWidth, img.naturalHeight)
  const cropX  = (img.naturalWidth  - srcMin) / 2
  const cropY  = (img.naturalHeight - srcMin) / 2
  ...
  return (
    <KonvaImage
      ...
      scaleX={1}
      scaleY={1}
      crop={{ x: cropX, y: cropY, width: srcMin, height: srcMin }}
