...
const btnActive  = { ...btnBase, background: '#1a1a2e', color: '#6c63ff', border: '1px solid #6c63ff' }
const btnSwapOn  = { ...btnBase, background: 'rgba(249,115,22,0.15)', color: '#f97316', border: '1px solid #f97316' }

function CanvasImage({ img, isSelected, isSwapFirst, onSwap, onSelect, swapModeRef }) {
  ...
  const handleClick = useCallback(() => {
    if (swapModeRef.current) onSwapRef.current(img.id)
    else onSelectRef.current(img.id)
  }, [img.id, swapModeRef])
  const w = img.naturalWidth  * img.scaleX
  const h = img.naturalHeight * img.scaleY
  const borderColor = isSwapFirst ? '#f97316' : isSelected ? '#6c63ff' : null
  const borderWidth = 3
  return (
    <>
      {/* Highlight rect drawn BEHIND image — does not affect hit area */}
      {borderColor && (
        <Rect
          x={img.x - borderWidth} y={img.y - borderWidth}
...
export default function App() {
  ...
  const swapModeRef  = useRef(false)
  ...
  // ── Keep refs in sync so Konva's cached handlers always read latest values ──
  useEffect(() => { swapFirstRef.current = swapFirst }, [swapFirst])
  useEffect(() => { swapModeRef.current  = swapMode  }, [swapMode])

  // ── Swap logic ──────────────────────────────────────────────────────────────
  // Reads swapFirstRef (not state) so it never goes stale in Konva event listeners
  const handleSwapTap = useCallback((id) => {
    const first = swapFirstRef.current
    if (!first) {
      selectImage(id)
      swapFirstRef.current = id
      setSwapFirst(id)
      return
    }
    if (first === id) {
      clearSelection()
      swapFirstRef.current = null
      setSwapFirst(null)
      return
    }
    // second tap — do the swap
    saveSnapshot()
    swapImages(first, id)
    swapFirstRef.current = null
    swapModeRef.current  = false
    setSwapFirst(null)
    setSwapMode(false)
    clearSelection()
  }, [selectImage, clearSelection, saveSnapshot, swapImages])

  const exitSwapMode = useCallback(() => {
    swapFirstRef.current = null
    swapModeRef.current  = false
    setSwapMode(false); setSwapFirst(null); clearSelection()
  }, [clearSelection])

  const toggleSwapMode = useCallback(() => {
    const next = !swapModeRef.current
    swapModeRef.current = next
    if (!next) { swapFirstRef.current = null; setSwapFirst(null); clearSelection() }
    setSwapMode(next)
  }, [clearSelection])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#0e0e1a', color: '#c8c8e8', overflow: 'hidden' }}>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          HEADER — clean mobile-first toolbar
      ══════════════════════════════════════════════════════════════════════ */}
      <header style={{ background: '#13131f', borderBottom: '1px solid #1e1e2e', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '0 12px 10px', flexWrap: 'wrap' }}>
          ...
          {/* Swap toggle */}
          <button style={swapMode ? btnSwapOn : btnGhost} onClick={toggleSwapMode}>
      ...
      <div id="canvas-container" style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#1a1a26', marginBottom: '36px' }}
        ...
        {/* Swap mode banner */}
        {swapMode && (
          <div style={{ position: 'absolute', top: '10px', left: '50%', transform: 'translateX(-50%)', zIndex: 100, display: 'flex', alignItems: 'center', gap: '8px', background: '#13131f', border: `1.5px solid ${swapFirst ? '#f97316' : '#6c63ff'}`, borderRadius: '10px', padding: '7px 14px', boxShadow: '0 4px 20px rgba(0,0,0,0.6)', whiteSpace: 'nowrap' }}>
            <span style={{ fontSize: '16px' }}>⇄</span>
            <span style={{ fontSize: '12px', fontWeight: 700, color: swapFirst ? '#f97316' : '#6c63ff' }}>
              {swapFirst ? 'Now tap the skin to swap with' : 'Tap a skin to select it'}
            </span>
            <button onClick={exitSwapMode} style={{ background: 'none', border: 'none', color: '#666', fontSize: '16px', cursor: 'pointer', lineHeight: 1, padding: '0 0 0 4px' }}>✕</button>
          </div>
        )}
          ...
          <Layer>
            ...
            {images.map(img => (
              <CanvasImage
                ...
                onSwap={handleSwapTap}
                onSelect={selectImage}
                swapModeRef={swapModeRef}
              />
            ))}
          </Layer>
        </Stage>
