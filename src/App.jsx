...
function CanvasImage({ img, isSelected, onSelect, isSwapFirst }) {
  ...
  // Store onSelect in a ref so Konva's cached listener always calls the latest version
  const onSelectRef = useRef(onSelect)
  useEffect(() => { onSelectRef.current = onSelect }, [onSelect])
  const handleClick = useCallback(() => onSelectRef.current(img.id), [img.id])
  return (
    <KonvaImage
      image={image}
      x={img.x} y={img.y}
...
export default function App() {
  ...
  const handleSwapTap = useCallback((id) => {
    ...
    import('react-hot-toast').then(({ default: toast }) => {
      toast('TAP: ' + id.slice(0,6) + ' first=' + (first ? first.slice(0,6) : 'null'), { duration: 2000 })
    })
    if (!first) {
      selectImage(id)
