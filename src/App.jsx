...
function CanvasImage({ img, isSelected, isSwapFirst, imgId }) {
  ...
  return (
    <KonvaImage
      ...
      strokeScaleEnabled={false} hitStrokeWidth={0} perfectDrawEnabled={false}
      listening={true}
    />
  )
}
...
export default function App() {
  ...
  // ── Keep refs in sync so Stage handler always reads latest state ──
  useEffect(() => { swapFirstRef.current = swapFirst }, [swapFirst])
  useEffect(() => { swapModeRef.current  = swapMode  }, [swapMode])

  // ── Single Stage-level click handler — never goes stale because it reads refs ──
  const handleStageClick = useCallback((e) => {
    const shape = e.target
    const stage = e.target.getStage()
    const clickedOnEmpty = shape === stage
    ...
    // Find which image was clicked by matching Konva node id
    const clickedId = shape.id()
    if (!clickedId) return

    if (swapModeRef.current) {
      // ── Swap mode tap ──
      const first = swapFirstRef.current
      if (!first) {
        selectImage(clickedId)
        swapFirstRef.current = clickedId
        setSwapFirst(clickedId)
      } else if (first === clickedId) {
        clearSelection()
        swapFirstRef.current = null
        setSwapFirst(null)
      } else {
        saveSnapshot()
        swapImages(first, clickedId)
        swapFirstRef.current = null
        swapModeRef.current  = false
        setSwapFirst(null)
        setSwapMode(false)
          ...
          <Layer>
            ...
            {images.map(img => (
              <CanvasImage
                ...
                isSwapFirst={swapModeRef.current && swapFirstRef.current === img.id}
              />
            ))}
          </Layer>
        </Stage>
