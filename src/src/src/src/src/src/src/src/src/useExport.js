...
export function useExport(stageRef) {
  ...
  const exportAs = useCallback(async (format = 'png', quality = 1) => {
    ...
    try {
      ...
      toast.loading('Rendering full resolutionâ€¦', { id: toastId })
      ...
      const blob = await exportLargeCanvas(
        ...
        (p) => {
          setExporting(true, p)
          toast.loading(`Exportingâ€¦ ${Math.round(p * 100)}%`, { id: toastId })
        }
      )
      ...
      toast.success(`Exported! ${canvasSize.width}Ã—${canvasSize.height}px â€¢ ${sizeMB} MB`, { id: toastId, duration: 5000 })
    } catch (err) {
