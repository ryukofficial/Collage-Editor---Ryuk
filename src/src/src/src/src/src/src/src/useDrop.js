...
export function useDrop() {
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    const loadingToast = toast.loading(`Loading ${imageFiles.length} image${imageFiles.length > 1 ? 's' : ''}â€¦`)
    ...
    try {
      const { results, errors } = await loadImageFiles(imageFiles, {
        batchSize: 6,
        onProgress: (p) => {
          toast.loading(`Loading imagesâ€¦ ${Math.round(p * 100)}%`, { id: loadingToast })
        },
      })
