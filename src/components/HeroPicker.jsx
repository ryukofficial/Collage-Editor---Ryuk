...
export default function HeroPicker({ onRegisterOpen }) {
  ...
  // ── Add to canvas — square 1:1 grid with cover scaling ──────────────────────
  const handleAddToCanvas = async () => {
    ...
    // Reposition existing images into the new square grid
    ...
    // MLBB skin images are portrait ~3:4 (300×400 is a safe assumption)
    const nw = 300, nh = 400
    const newImages = selectedSkins.map((skin, i) => {
      const globalIndex = images.length + i
      const scale = Math.max(cellSize / nw, cellSize / nh)
      return {
        src:           skin.image,
