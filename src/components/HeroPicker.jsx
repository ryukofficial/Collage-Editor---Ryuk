...
const TIER_STYLES = {
  ...
  "Valentine's Premium": { bg: '#2a0010', border: '#fb7185', text: '#fda4af', icon: '💝' },
  'Licensed':            { bg: '#001520', border: '#0ea5e9', text: '#bae6fd', icon: '🎤' },
  'Anniversary':         { bg: '#1a1000', border: '#f59e0b', text: '#fde68a', icon: '🎉' },
  'All Star':            { bg: '#1a1a00', border: '#eab308', text: '#fef08a', icon: '⭐' },
  'AS':                  { bg: '#001a1a', border: '#14b8a6', text: '#99f6e4', icon: '🎯' },
  'MCGG':                { bg: '#0a0a2a', border: '#818cf8', text: '#c7d2fe', icon: '🎮' },
}
...
const SERIES_MAP_RAW = {
  ...
  "K'": "KOF", "Orochi Chris": "KOF", "Kula Diamond": "KOF",
  "Gusion KOF": "KOF", "KOF": "KOF",
  // ALLSTAR
  "SPARKLE Melissa": "ALLSTAR", "SPARKLE Fredrinn": "ALLSTAR", "SPARKLE Estes": "ALLSTAR",
  // All Star
  "Hanabi Moonlit Ninja": "All Star", "Moskov Wrymlord": "All Star", "Moskov Wyrmlord": "All Star",
  // Attack on Titan
  ...
  "Summer Blast": "Summer",
  // Golden Month
  "Crescent Dervish": "Golden Month", "Neonscape Icon": "Golden Month",
  // Anniversary
  "Angelic Fan": "Anniversary", "Moon Priestess": "Anniversary", "Starblazer": "Anniversary",
  // All Star
  "Tidescale Sealord": "All Star", "Moonlit Ninja": "All Star", "Infernal Wrymlord": "All Star", "Infernal Wyrmlord": "All Star",
  // AS Skins
...
// Build a cached array of [normalizedKey, series] for fuzzy lookup
const SERIES_MAP_ENTRIES = Object.entries(SERIES_MAP) // keys already lowercased

// Returns the series for a given heroId + skinName
// Uses: 1) exact lowercase match, 2) substring match, 3) keyword overlap match
function getSeries(heroId, skinName) {
  const excludeKey = `${heroId}__${skinName}`
  if (HERO_SERIES_EXCLUDE.has(excludeKey)) return null

  const needle = skinName.toLowerCase().trim()

  // 1. Exact match
  if (SERIES_MAP[needle]) return SERIES_MAP[needle]

  // 2. Substring match — skin name contains map key or map key contains skin name
  for (const [key, series] of SERIES_MAP_ENTRIES) {
    if (needle.includes(key) || key.includes(needle)) return series
  }

  // 3. Keyword overlap — split both into words, check if enough words match
  const needleWords = needle.split(' ').filter(w => w.length >= 3)
  if (needleWords.length > 0) {
    let bestSeries = null, bestScore = 0
    for (const [key, series] of SERIES_MAP_ENTRIES) {
      const keyWords = key.split(' ').filter(w => w.length >= 3)
      if (keyWords.length === 0) continue
      const matched = needleWords.filter(w => keyWords.includes(w)).length
      const score = matched / Math.max(needleWords.length, keyWords.length)
      if (score > bestScore && score >= 0.5) { bestScore = score; bestSeries = series }
    }
